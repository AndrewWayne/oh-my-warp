...
- Terminal sessions backed by `tmux` control mode (durable, attach/detach, resize).
...
**Solution.** A standalone `omw-agent` binary with:

- Pluggable LLM providers (§5.1).
- Pluggable tools via **MCP** (filesystem, git, GitHub, Linear, custom servers — anything that speaks MCP).
- Built-in tools: shell (with approval), file I/O, grep, editor.
- **ACP server mode** so other editors (Zed, Helix-with-ACP, etc.) can use `omw-agent` as their backend.
- Approval policy engine: `read_only` / `ask_before_write` / `trusted`, plus per-command allow/deny lists.
- Persistent transcripts in SQLite, queryable + exportable.
...
**Tradeoff.** v1 ships the agent as a separate binary (`omw-agent`) with both a CLI and an HTTP/WebSocket API. The forked Warp UI calls into it via the local server. This is more code than embedding the agent in the client, but lets us reuse it from CLI, Zed, the Web Controller, and headless CI.
...
- FR-4.4 Sessions are tmux-backed and survive `omw remote` restarts.
...
│   Rust           │                │   Rust                     │
│   Providers      │                │   tmux control mode        │
│   MCP tools      │                │   PTY sessions             │
│   ACP server     │                │   Pairing, signed reqs     │
│   Cost reconcile │                │   Web Controller bundle    │
└──────┬───────────┘                └──────────────┬─────────────┘
       │                                           │
...
| `omw-agent` | Rust | Agent loop, providers, tools, ACP server, cost | ✓ |
| `omw-remote` | Rust | tmux-backed sessions + remote API + pairing | ✓ |
...
|---------|------------|-------|
| Agent session lifecycle (create, run, end) | `omw-agent` | Sessions, transcripts, tool-call rows. |
| Provider API calls (HTTP, streaming, tokenization) | `omw-agent` (provider crates) | One client per provider; retry/backoff isolated. |
| MCP server lifecycle | `omw-agent` | Spawn, JSON-RPC, tool registry. |
| Approval policy *logic* | `omw-policy` (library, no state) | Pure function: (mode, context, allowlist) → decision. |
| Approval *decisions* (recorded) | `omw-agent` | Calls `omw-policy`, writes to `approvals` table, emits audit entry. |
| Cost estimation | `omw-agent` | Tokenizer-side estimate at request time. |
| Cost reconciliation | `omw-agent` | API-reported usage joined to estimate; delta logged. |
| PTY/tmux sessions | `omw-remote` | tmux control-mode is the only path; no raw spawn in v1. |
...
oh-my-warp/                  # repo / codename
  ...
  crates/
    ...
    omw-server/
    omw-agent/
    omw-provider-{openai,anthropic,openai-compat,ollama}/
    omw-remote/
    omw-config/
    omw-policy/
    omw-keychain/
...
- `pty_sessions(id, name, tmux_session, created_at, last_active)`
...
`omw-agent` CLI: provider trait, streaming chat, model listing. Tier-1 providers (OpenAI, Anthropic, OpenAI-compatible, Ollama). `omw ask` and `omw agent`. SQLite transcripts. Cost reporting per response, per session, per day. Keychain integration. `omw-config` and `omw-keychain` crates wired in.
...
`omw-pty` over `portable-pty`. `omw-remote` with tmux control mode. HTTP + WS API per the spec. Pairing flow (QR, Ed25519 keypair, signed requests, replay window). Web Controller (`apps/web-controller/`): pairing, terminal, agent, approvals, diff. Audit "Activity" view in the forked client. Single-host scope only.

**Exit:** I pair a single host, attach to a tmux session over Tailscale Serve, ask agent something, approve a write, see the audit entry. Protocol review sign-off in repo.
