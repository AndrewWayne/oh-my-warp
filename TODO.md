...
- [x] **Gap 1**: `omw-server::SessionRegistry` external-source variant + `PtyController` I/O tap in warp-stripped + per-pane "Share this pane" UI. (~2-3 hours estimated; landed across A+B+C by 2026-05-09.)
  ...
  - [x] Part C — auto-share-on-Phone-click wiring. The original 2026-05-09 implementation invoked `share_self_pane` from the footer's `ToggleOmwPair` arm and attached to the active pane without creating a sibling shell. The 0.0.12 candidate replaces that temporary footer-only entry point with one canonical per-pane action exposed in the pane header, overflow menu, and Command Palette. The self-contained Windows package and automated gates pass; desktop visual and physical-phone acceptance remain pending.
...
- [x] **Gap 4**: `omw/tailscale.rs` module + multi-origin pinning + orchestration (commits 6eb3287, 9f75c1f, 5e7b06c; 71 → 74 omw-remote tests, 4 tailscale unit tests). `pinned_origin: String` → `pinned_origins: Vec<String>` (3 contract tests in `ws_origin_multi.rs`). Tailscale `detect_status` / `serve_https` / `unserve` shell out via `std::process::Command` with explicit args. **Shipped behaviour (corrected 2026-07-14, issue #9): direct tailnet IP, not Serve.** `OmwRemoteState::start` orchestrates: bind → detect → assemble pair URL from the **direct tailnet IPv4** origin (`http://100.x.y.z:8787`) when the daemon is exposed via `OMW_REMOTE_BIND` → set pinned origins to `[loopback, tailnet-ip]`. It does **not** call `serve_https`, `tailscale_serving` is hard-coded `false`, and `stop` does **not** call `unserve`. Rationale: plain HTTP over the tailnet carries the pairing flow and the signed WS, whereas `tailscale serve --bg` can block for seconds on first-use cert provisioning and previously hung the UI thread. In the default loopback mode the pair URL stays loopback (a tailnet URL would be unreachable). `serve_https`/`unserve` are retained, uncalled, for a future HTTPS path gated behind an explicit opt-in (e.g. `OMW_TAILSCALE_SERVE=1`) — restore the `stop`-time `unserve` call with it.
...
- [ ] **iOS Safari + Tailscale cold-path connect delay.** First WS handshake to a peer can stall 10–30s when the WireGuard path / iOS connection pool is cold (no recent packets). Mitigated by client-side retry-with-timeout (3 × 6s) and an HTTP pre-warm fetch to `/api/v1/host-info` before the WS upgrade (`packages/byorc-client/src/pty-ws.ts:302-317`), but not eliminated. Empirical signature: clicking "← Sessions" then auto-tracing back to the same terminal connects instantly (warm path); entering fresh from session list always times out and then succeeds on the retry. Needs a Tailscale-side diagnostic (`tailscale ping`, `tailscale netcheck`) — not an app-level bug.
...
- [ ] **Whole-workspace auto-share (design notes; helper removed 2026-07-14).** The persistent phone action remains explicitly per-pane and shares only the current pane (`omw::pane_auto_share::share_self_pane`); whole-workspace sharing is still deferred. A `share_all_local_panes` helper once walked the current window's workspace and shared every `local_tty`-backed pane, but it **crashed warp-oss on Phone click in two separate attempts** (commits `1272ce6`, `49ffbb2`): it re-entered every foreign `TerminalView`'s update closure via `PaneGroup::for_all_terminal_panes`, nesting PaneGroup borrows. It sat unused and compiled — vendor `cargo check` flagged it as dead code — so it was deleted per issue #18 rather than left as a re-introduction hazard; recover it from git history if useful. Design notes for whoever revisits this:
  - **Do not re-enter foreign views.** The crash was iteration + nested borrow, not sharing itself. Collect the io handles first and issue `share_pane` outside the per-view closure, or drive it from a level that already owns the pane tree.
  - **Reactive add/remove**: panes opened *after* daemon start were never auto-shared (one-shot enumeration). Subscribing to `PaneStackEvent::ViewAdded`/`::ViewRemoved` per stack works but isn't surgical (every PaneGroup × every PaneStack); a periodic re-walk is the cheaper alternative.
  - **Multi-window**: the old helper only covered the window the user clicked from. `WorkspaceRegistry::all_workspaces` makes the multi-window variant straightforward.
  - **Only `local_tty::TerminalManager` panes are shareable** — remote SSH, shared-session-viewer, and mock managers lack the `event_loop_tx`/`pty_reads_tx` pair (see `local_io_handles_for`).
  - **Prerequisite**: a GUI harness. Root CI never compiles `vendor/warp-stripped/`, so a Phone-click crash cannot be caught by the test suite — that is precisely how this shipped twice.
