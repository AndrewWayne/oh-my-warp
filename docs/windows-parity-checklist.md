...
Status: Phase 5M E5BD/0720 package; bounded desktop OMW-02 and local Windows integration PASS; hosted Windows CI run pending
...
| OMW-09 | P1 | The embedded Web Controller passes Vitest, is served by the Windows host, and lets a physical phone pair with the same active PowerShell pane, exchange I/O, disconnect, and reattach without creating an unrelated shell. | Run `npm.cmd test --workspace @oh-my-warp/web-controller`, follow `docs/mobile-web-controller-phone-qa.md`, and record served assets, pane/process identity, byte flow, disconnect, and reconnect. | `BLOCKED`: the Web Controller build passed and Vitest recorded 114 passed with 1 skipped. Embedded serving, Tailscale, and the physical-phone Windows lane remain unexercised. | M5 |
...
- The current release package is ZIP `072066AB...D1C33`, containing exact EXE
  `E5BD8769...4303C`. Independent verification passed all 16,759 manifest
  hashes, archive-safety and required-entry checks, package-lock preservation,
  PDB exclusion, and all eight forbidden-hostname checks. Initial E5BD/DE95
  passed clean-PATH helper/kernel probes; 0720 retained those helper/runtime
  bytes and exercised the refreshed kernel in the exact-package agent flow.
