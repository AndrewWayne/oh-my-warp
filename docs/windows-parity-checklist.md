...
Status: local 0.0.12 6099/0B2E/9278 dogfood package and installer ready; hosted PR CI passes; the recorded physical-iPhone/Tailscale pane-sharing scope passed on CBBF; tagged release, fresh-account/VM, and broader display validation remain deferred
...
| omw reference | PR-head commit `f5e52613c662e49be3cea01ef6c7ebfa6328d34e`, merged to upstream `main` as `f0fb01c7d55c1165e29fc72664d662a6dba4abac` |
...
| Current 0.0.12 release candidate | Release EXE `609990919E218E0370C31D3575BD442F1BE4DAD3A7960953D3BD0CFFCCE7BAFE`, 295,903,232 bytes, staged at `dist/staging-v0.0.12-dogfood.pane-phone.20260827-windows/omw-warp-oss.exe` and embedded byte-identically in ZIP `0B2E217E1090E3AF00D2481EB502C9C62CE1462A29260E05B22578735B4275A2`, 190,236,842 bytes. Per-user installer `92780C3AD7258731CC4074DFE2D8D4C2030828447A81470F9492B4541DE6EFCA`, 131,727,903 bytes, embeds the same manifest-verified 16,769-file payload and generates an installed `Uninstall.exe`. Formal ZIP verification passed 16,769 files, 65 directories, and 487,389,308 payload bytes; clean-PATH helper/kernel probes, PDB exclusion, package-lock preservation, LF-only checksum sidecars, and independent source/staged forbidden-hostname audits pass. Installer first-install, locked-upgrade, replacement-upgrade, full installed hashes, uninstall, and residue cleanup pass. The preference-preserving migration is embedded, the old destructive width update and old unredacted pair-URL log string are absent. The recorded physical-iPhone/Tailscale pane-sharing scope passed on predecessor EXE CBBF; final commit `f5e5261` is limited to preference-preserving migration behavior and pair-URL log redaction and passes its automated tests and hosted PR CI. Canonical tagged-release artifacts will be rebuilt by GitHub Actions and will have different hashes. |
...
| WI-11 | P0 | The runtime directory contains the executable, ConPTY, DirectX compiler libraries, and bundled OpenConsole required for local launch. | Verify `warp-oss.exe`, `conpty.dll`, `dxcompiler.dll`, `dxil.dll`, and `x64\OpenConsole.exe`, then launch from that runtime directory. | `PASS` 2026-08-27: final dogfood ZIP `0B2E217E...B4275A2` contains EXE `60999091...E7BAFE`, all four native runtime components, `vcruntime140.dll`, bundled Node, agent, and keychain helper. Formal verification passed all 16,769 manifest hashes, 65 directory entries, required entries, duplicate/safe-path checks, package-lock preservation, and PDB exclusion. Clean-PATH probes confirmed the packaged helper and agent kernel use only bundled or standard Windows dependencies. Clean-machine behavior remains WI-12. | N/A |
| WI-12 | P0 | A Windows preview package launches on a clean standard-user machine without Rust, Cargo, npm, Node, Visual Studio, or developer PATH entries. | Extract the package in a fresh Windows account/VM, verify hashes, launch from Explorer, and execute CT-03 through CT-07. | `NOT TESTED` for the required fresh-account/VM lane. The final self-contained ZIP `0B2E217E...B4275A2` with EXE `60999091...E7BAFE` passed all 16,769 manifest hashes, archive/runtime checks, no-cloud audit, packaged Node syntax, and clean-PATH helper/kernel probes. Current-account GUI and installer acceptance do not substitute for the required fresh-account/VM Explorer launch and CT-03 through CT-07, which remain deferred. | N/A |
| WI-13 | P1 | Windows CI builds and runs the selected Rust, TypeScript, web, ConPTY, and no-cloud gates with retained logs. | Inspect a clean Windows CI run for the commands referenced by this checklist and verify artifacts/evidence are retained. | `PARTIAL PASS; TAGGED RELEASE AUDIT PENDING` 2026-08-27. Hosted run [33033991754](https://github.com/AndrewWayne/oh-my-warp/actions/runs/33033991754) completed all 12 PR jobs successfully at head `f5e5261`, including `Windows / root + TS integration`. The job ran the selected root Rust, ConPTY, native Credential Manager/helper, agent, Web, and BYORC gates and retained the `windows-ci-logs` artifact. The compiled-GUI/no-cloud audit remains a tagged-release gate, so this full row becomes `PASS` only after that workflow and its retained evidence succeed. The 12 PR-workflow annotations are Node.js action-runtime deprecation warnings, not test failures. | N/A |
| WI-14 | P0 | A standard user can install, upgrade, and uninstall omw through executable Windows surfaces without developer tools or elevation; uninstall preserves user data and credentials by default. | Build `setup.exe` from a manifest-verified staging payload, install silently into a disposable path, compare all installed hashes, require Apps & Features and Start Menu entries, reject a locked upgrade without damage, replace the whole payload on an unlocked upgrade, then run the generated uninstaller and require binaries/shortcuts/registration to be absent. | `PASS` 2026-08-27 against installer `92780C3A...E6EFCA`, 131,727,903 bytes, built from the final 0.0.12 dogfood staging payload. The smoke installed and re-hashed all 16,769 manifest entries, checked product metadata and shortcut targets, returned exit 12 on a write-locked executable without changing it, removed a deliberately stale file during replacement upgrade, re-hashed the upgraded payload, and removed the disposable install directory, Apps & Features key, and Start Menu folder through `Uninstall.exe /S`. No registry, shortcut, process, or temporary-directory residue remained. The NSIS script contains no user-data or Credential Manager deletion path. Installer signing remains deferred. | N/A |

## omw features

| ID | Pri | Target behavior | Reproducible Windows test | Current Windows status and evidence | Mac ref |
|---|---|---|---|---|---|
| OMW-01 | P0 | The local executable contains none of the eight forbidden Warp/Firebase hostnames. | Run `bash scripts/audit-no-cloud.sh target/debug/warp-oss.exe` with MSYS2 GNU `strings`; require eight zero counts and `audit-no-cloud: OK`. | `PASS` 2026-08-27: final dogfood EXE `60999091...E7BAFE` passed the fail-closed eight-hostname GNU strings audit, and the independent staged-package audit found zero occurrences for all eight patterns. The packaged agent and helper also passed their clean-PATH probes. The upstream Phase 2 binary contained 20 total hits across five patterns. | N/A |
...
| OMW-09 | P1 | The embedded Web Controller passes Vitest, is served by the Windows host, and lets a physical phone pair with the same active PowerShell pane, exchange I/O, disconnect, and reattach without creating an unrelated shell. | Run `npm.cmd test --workspace @oh-my-warp/web-controller`, launch the packaged GUI with an explicit tailnet `OMW_REMOTE_BIND` and `OMW_REMOTE_ALLOW_DEFAULT_WRITE=1`, then follow `docs/mobile-web-controller-phone-qa.md`. Begin in an ordinary PowerShell pane before starting an agent and require the persistent pane-header action; then start normal `codex` and require the action in alternate-screen mode. Repeat with the write variable unset and a fresh browser identity. Record pair capabilities, served assets, pane/PID and process-tree identity, per-pane labels, byte flow, disconnect, reconnect, unshare, and the read-only default. | `PARTIAL PASS` 2026-08-27: exact-value write-opt-in tests, the focused 8/8 phone-action suite, Web Controller Vitest (114 passed, 1 intentional skip), and the production bundle/package gates pass. Physical-iPhone acceptance over Tailscale confirmed the persistent pane-sharing UI, same-pane identity, bidirectional input/output, reconnect, unshare, and the secure read-only default. The physical record does not separately establish every alternate-screen, PID/process-tree, or no-sibling-shell assertion in this full row. Final commit `f5e5261` is limited to preference-preserving migration behavior and pair-URL log redaction, with its remote-state tests and hosted Windows CI passing. This is direct tailnet-IP acceptance, not Tailscale Serve acceptance under OMW-07. | M5 |
...
1. Run the canonical tagged release workflow for both platforms and inspect its
   retained Windows build/audit evidence before accepting the published assets.
...
3. Keep OMW-07 Tailscale Serve and OMW-10 reverse-resize behavior explicit as
   deferred gaps; the direct-tailnet physical-phone acceptance scope is complete.
...
Confirmed through the final 0.0.12 dogfood candidate:
...
- The current 0.0.12 package is ZIP `0B2E217E...B4275A2`, containing exact EXE
  `60999091...E7BAFE`. Independent verification passed all 16,769 manifest
  ...
  kernel probes. The recorded physical-iPhone/Tailscale pane-sharing scope
  passed on predecessor EXE CBBF; final `f5e5261` changes are limited to
  migration preservation and log redaction and pass automated/hosted checks.
  Broader desktop visual and fresh-account/VM coverage remain deferred.
...
Final PR #132 and 0.0.12 candidate result:

- Commit `f5e5261` preserves existing `warp_ai_width` preferences and removes
  the pairing URL/token from desktop logs. Its SQLite migration test, Windows
  `omw_local` build path, 14 remote-state tests, and `git diff --check` pass.
- Hosted CI run `33033991754` completed all 12 jobs successfully, including
  `Windows / root + TS integration`. Its 12 annotations are action-runtime
  Node.js deprecation warnings rather than product or test failures.
- The final dogfood candidate is EXE `60999091...E7BAFE`, ZIP
  `0B2E217E...B4275A2`, and installer `92780C3A...E6EFCA`. Formal ZIP
  verification and the complete installer lifecycle pass, both checksum
  sidecars are ASCII/LF-only, packaged-source provenance passes, and cleanup
  leaves no install, registry, shortcut, process, or temporary residue.
- PR #132 merged the tested tree to upstream `main` as `f0fb01c`. The official
  tagged release must rebuild from the tag; these dogfood hashes must not be
  uploaded or reused as canonical release hashes.

Remaining blocking or deferred gaps after the final 0.0.12 dogfood checkpoint:
...
- `P0`: OMW-02 desktop Settings, terminal, and local-agent network-capture scope
  passes. Functional direct-tailnet phone acceptance is recorded under OMW-09,
  but OMW-02 Phone/Tailscale network capture, UDP/DNS attribution, and elevated
  process-filtered ETW/WFP coverage remain deferred.
- `P0`: exact-binary OMW-04 and OMW-05 direct GUI evidence is on predecessor B37.
  Later candidates omit listener 9277, repair keyless Ollama sessions, add the
  final Windows UI/package work, preserve widths, and redact pair URLs; record a
  scoped-change waiver or rerun if policy requires every P0 GUI row on one hash.
- `P1`: the documented Tailscale Serve flow and the current direct tailnet-IP
  implementation do not match.
- `P1`: the Windows test lane passes on a clean hosted runner and retains its
  CI logs. The tagged release workflow still needs to produce and publish the
  canonical 0.0.12 artifacts and release-build audit evidence.
...
| 20260827-pr132-hosted-ci | 2026-08-27 | PR head `f5e5261`; merged tree `f0fb01c` | GitHub-hosted Windows/macOS/Linux runners | WI-13 hosted PR scope; root Rust/ConPTY/keychain; agent; Web; BYORC | `PASS` for PR CI scope; tagged release audit pending | [Actions run 33033991754](https://github.com/AndrewWayne/oh-my-warp/actions/runs/33033991754) | All 12 PR jobs passed, including the 8m40s Windows root + TS integration job. The only annotations were 12 Node.js action-runtime deprecation warnings. The compiled release/no-cloud gate remains in the tagged workflow. |
| 20260827-pane-phone-physical | 2026-08-27 | physical lane on EXE `CBBF6485...650CA` | Win 22631.6199 x64; physical iPhone over Tailscale | OMW-09 accepted scope | `PASS` for recorded scope | [PR #132 acceptance record](https://github.com/AndrewWayne/oh-my-warp/pull/132) | Persistent pane action, same-pane identity, bidirectional input/output, reconnect, unshare, and read-only default passed. Alternate-screen and complete process-tree assertions remain governed by the full OMW-09 row. |
| 20260827-final-release-r2 | 2026-08-27 | EXE `60999091...E7BAFE`; ZIP `0B2E217E...B4275A2`; installer `92780C3A...E6EFCA` | Win 22631.6199 x64 | WI-11, WI-14, OMW-01; final provenance | `PASS`; unsigned dogfood candidate | `dist/final-validation-0.0.12-dogfood.pane-phone.20260827/` | Formal ZIP verification passed 16,769 files/65 directories/487,389,308 payload bytes. Installer first install, locked exit 12, replacement upgrade, full hash comparisons, `Uninstall.exe`, and residue cleanup passed. Both sidecars are ASCII/LF-only; staged and built EXE hashes match; new migration text is present and the stale unredacted pair-URL log string is absent. Canonical tagged-release rebuild remains required. |
