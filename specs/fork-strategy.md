...
2. **AGPL header enforcement** — resolved 2026-07-13 (issue #6). The `vendor-omw-headers` CI check enforces the header on every omw file under `vendor/warp-stripped/**/omw/`; its failure message prints the exact preamble to prepend. A generator was deemed unnecessary given the check.
