...
| I-6 | New paired device defaults to `read_only`. Upgrade requires explicit user action on the host. | `pair/redeem` mints `[pty:read, agent:read, audit:read]` by default; `pty:write` only when the host opted in via `omw remote start --allow-default-write`. Per-device `omw pair upgrade` deferred to #23. | `pair_redeem_grants_write_when_opted_in` + read-only default-caps assertion in `http_pair_redeem`. |
