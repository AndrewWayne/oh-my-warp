...
impl CodeFooterView {
    ...
    pub fn new(path: PathBuf, ctx: &mut ViewContext<Self>) -> Self {
        ...
        if Self::is_tab_config_path(&path) {
            let tab_config_skill_button = ChannelState::official_cloud_services_enabled()
                .then(|| Self::create_tab_config_skill_button(ctx));
            let mut footer = Self {
...
impl View for CodeFooterView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.is_tab_config_footer() {
            ...
            let status_text = if self.tab_config_skill_button.is_some() {
                "Use Oz to update this config"
            } else {
                "Tab config"
            };
            footer_content.add_child(
                Shrinkable::new(
                    1.,
                    Self::render_status_text(theme, appearance, status_text.to_string()),
                )
                .finish(),
            );
