...
impl SettingsView {
            ...
            nav_items = nav_items
                ...
                .filter_map(|item| match item {
                    SettingsNavItem::Page(section) => section
                        .is_visible_in_omw_local_mode()
                        .then_some(SettingsNavItem::Page(section)),
                    SettingsNavItem::Umbrella(mut umbrella) => {
                        umbrella
                            .subpages
                            .retain(SettingsSection::is_visible_in_omw_local_mode);
                        (!umbrella.subpages.is_empty())
                            .then_some(SettingsNavItem::Umbrella(umbrella))
                    }
                })
                .collect();
        }
    ...
    fn should_render_page(&self, settings_page: &SettingsPage, app: &AppContext) -> bool {
        if ChannelState::omw_local_mode() && !settings_page.section.is_visible_in_omw_local_mode() {
            return false;
        }
