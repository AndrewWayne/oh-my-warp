...
impl ChannelState {
    pub fn init() -> Self {
        ...
        Self::apply_omw_local_config(channel, &mut config);
    ...
    fn apply_omw_local_config(channel: Channel, config: &mut ChannelConfig) {
        if Self::should_disable_official_cloud_for(channel) {
    ...
    pub fn should_disable_official_cloud_for(channel: Channel) -> bool {
        cfg!(feature = "omw_local") || matches!(channel, Channel::Oss)
    }

    pub fn omw_local_mode() -> bool {
        Self::should_disable_official_cloud_for(Self::channel())
    }

    pub fn official_cloud_services_enabled() -> bool {
        !Self::omw_local_mode()
    }
